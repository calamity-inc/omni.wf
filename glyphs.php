		...
		<div class="row">
			<div class="col-6">
				...
				<select class="form-control" id="filter">
					...
					<option value="1">Universal Promo Code</option>
					<option value="2">Some Effort Required</option>
				</select>
			</div>
			<div class="col-6">
	...
	<script>
		...
		function updateList()
		{
			...
			.filter(([uniqueName, item]) =>
			{
				...
				&& (filter != 1 || supplementalGlyphData[uniqueName]?.promo_code)
				&& (filter != 2 || (
			...
			.forEach(([uniqueName, item]) =>
			{
				...
				glyph.href = "/#q=" + encodeURIComponent(dict[item.name] ?? uniqueName);
				glyph.target = "_blank";
