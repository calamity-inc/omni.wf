	...
	<script>
		...
		function resolveTagsToUses(results)
		{
			...
			for (const result of results)
			{
				if (result.type == "tag")
				{
					let entry = Object.entries(ExportWarframes).find(([uniqueName, item]) => item.name == result.key);
					if (entry)
					{
						res.push({ type: "warframe", key: entry[0], value: entry[1] });
						continue;
					}
					entry = Object.entries(ExportWeapons).find(([uniqueName, item]) => item.name == result.key && item.totalDamage != 0);
					if (entry)
					{
